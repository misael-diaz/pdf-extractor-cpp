...
The drive here is to write exploratory code to experiment with libpoppler to extract data from medical records. The challenge is that the records come from various institutions and
...
- **filters out watermark**: at the byte level the watermarks are easy to identify, filtering out the watermark was achieved by writing simple code that the CPU can predict (performant). These were applied after doing the ascii folding transformation.
- **extracts names from multiline text**: extracts the full patient name even if it spans multiple lines (as in the tabulated Sura records).
...
The build is simple, all that you need is to install poppler, this document shows you how, and to build with the C++ compiler provided by GCC or LLVM if you prefer. Simplicity is an advantage, you don't have to deal with massive dependencies. Sometimes dependency handling entails compartmentalization with Docker to isolate the application and that is more work and introduces yet another layer to be concerned about to deploy the solution. The build time is reasonable if you compare it against the time it requires to build Rust applications. (There's the problem of Rust's borrow checker which makes the development much bumpy and strenous, if you are looking for development speed unless you are a pro Rust developer choose something else and that's what I did.) This is a lean implementation so even if a C++ project is not considered to be fast build project in general, this one is the exception because we do not leverage the complexities introduced by the STL.
