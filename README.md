...
The build is simple, all that you need is to install poppler, this document shows you how, and to build with the C++ compiler provided by GCC or LLVM if you prefer. Simplicity is an advantage, you don't have to deal with massive dependencies. Sometimes dependency handling entails compartmentalization with Docker to isolate the application and that is more work and introduces yet another layer to be concerned about to deploy the solution. The build time is reasonable if you compare it against the time it requires to build Rust applications. (There's the problem of Rust's borrow checker which makes the development much bumpy and strenous, if you are looking for development speed unless you are a pro Rust developer choose something else and that's what I did.) This is a lean implementation so even if C++ is not considered a fast build project for this one it is because we do not leverage the complexities introduced by the STL.
